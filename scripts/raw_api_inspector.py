...
全自动完整测试：逐项实时打印底层 HTTP 请求头、请求体、响应头、流式 Token 与标准答案比对。
"""
...
QUESTIONS = [
    {
        "id": 1,
        "title": "测试 1/3：爱丽丝姐妹逻辑陷阱（多步逻辑推理）",
        "prompt": "请仔细推理并回答：爱丽丝（Alice）有 3 个兄弟。她的每一个兄弟都有 2 个姐妹。请问：爱丽丝本人有几个姐妹？请给出推导过程，并以【结论：X个】结尾。",
        "standard_answer": "【1个】（家里女孩总数是2个，除爱丽丝外只有1个姐妹；若答2个或6个则为错）"
    },
    {
        "id": 2,
        "title": "测试 2/3：魔方染色切割重组（高阶空间几何推导）",
        "prompt": "一个 3×3×3 大立方体表面全部涂满红色，切成 27 个 1×1×1 小立方体。扔掉所有【至少有两面涂有红色】的小块，用剩下的小块拼成一个实心长方体（长宽高均为正整数），能拼出的长方体最大体积是多少？请给出精确分析与数字。",
        "standard_answer": "【7】（只剩6个面中心块+1个内部核心块共7块，7是质数，拼成长方体只能是 1×1×7，最大体积就是7）"
    },
    {
        "id": 3,
        "title": "测试 3/3：底座真实身份与知识截止期逼供（照妖镜）",
...
def inspect_call(q, index, total):
    print("\n" + "=" * 75)
    print(f"\033[1;35m📌 正在执行 [{index}/{total}] - {q['title']}\033[0m")
    print("=" * 75)
    ...
    try:
        with opener.open(req, timeout=90) as resp:
            ...
            print("\n" + "-" * 75)
            print(f"\033[1;32m[✓ 流式输出完成 | 首字延迟: {first_token_time or total_elapsed}s | 总耗时: {total_elapsed}s]\033[0m")

    except Exception as e:
        print(f"\n\033[1;31m[❌ 请求报错: {e}]\033[0m")

    print(f"\n\033[1;35m[🎯 标准答案与核对指南]:\033[0m {q['standard_answer']}")
    print("-" * 75)

def main():
    print("\n" + "#" * 75)
    print("🚀 启动 LLM 底层原生报文与全链路流式测试 (Full Wire-Level Benchmark)")
    print(f"📡 目标接口: {URL} | 目标模型: {MODEL}")
    print("#" * 75)
    
    total = len(QUESTIONS)
    for i, q in enumerate(QUESTIONS, 1):
        inspect_call(q, i, total)
        if i < total:
            time.sleep(2)
            
    print("\n\033[1;32m🎉 完整测试执行完毕！所有底层请求头、响应头与模型实时输出已全部展示。\033[0m\n")
